...
| `tests/sam2-mirador.probe.spec.ts` | The `$.SamLocal` tool live in Mirador, via the harness template `rsp:SamLocalTest` (runtime layer). Reports the WebGPU toolbar gate, drives point → mask preview → double-click commit → Save, then queries the store for the persisted region. Writes one `EX_Digital_Image_Region` labelled "SamLocal probe region" per run. Needs the stack + GPU launch flags (in the spec). |
